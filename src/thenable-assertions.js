...
export const createPromiseResolvedIn = (...args) => {
	...
	return new Promise(
		(resolve) => {
			setTimeout(
				() => args.length > 1 ? resolve(value) : resolve(),
				ms
			)
		}
	)
...
export const assertResolvedAround = (thenable, expectedMs, maxGap) => {
	...
	return Promise.race([
		exports.createPromiseResolvedIn(maxDuration).then(
			() => timeout
		),
...
export const assertResolved = (thenable) => thenable.then(
	(value) => value,
	(reason) => assert.fail(`has rejected with ${reason}`, 'expected to resolve')
)
export const assertRejected = (thenable) => thenable.then(
	() => {
