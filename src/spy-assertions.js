...
export const assertCalled = (...args) => args.forEach(
	(arg) => assert(
		getLastCallWhenSpy(arg).wasCalled(),
		null,
		'was not called'
	)
)

export const assertNotCalled = (...args) => args.forEach(
	(arg) => assert(
		getLastCallWhenSpy(arg).wasCalled() === false,
		null,
		'was called'
	)
)
...
export const assertCalledWith = allowSpy(
	(call, ...args) => {
		assertCalled(call)
...
export const assertCalledIn = allowSpy(
	(call, expectedMs) => {
		assertCalled(call)
...
export const assertCalls = (spy, ...expectedCallsArgs) => {
	...
	expectedCallsArgs.forEach(
		(expectedCallArgs, index) => assertCalledWith(spy.getCall(index), ...expectedCallArgs)
	)
}
